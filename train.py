...
def train(hyp, opt, device, callbacks):  # path/to/hyp.yaml or hyp dictionary
    ...
    if FRACTIONFISH:
        ...
        # Fractionfish: we need to weigh all the pieces equally, as all of
        # them are important, if I weigh the eight tile less, then
        # detections might not be good for the eight tile
    ...
    for epoch in range(
        ...
        if RANK in [-1, 0]:
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                if (
                    (epoch > 0) and
                    (opt.save_period > 0) and
                    (epoch % opt.save_period == 0)
                ):
